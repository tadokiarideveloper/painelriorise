...
export async function onRequestGet({ request, env }) {
  ...
  const result = await env.DB.prepare(`
    SELECT
      ...
      u.created_at,
      u.updated_at,
      COUNT(p.id) AS punishment_count
    FROM users u
    LEFT JOIN punishments p ON p.created_by_username = u.username
    GROUP BY u.id
    ORDER BY u.role_level DESC, u.nickname ASC
  `).all();

  const users = (result.results || []).map(u => ({
    ...u,
    role_name: roleName(u.role_level),
    password: undefined
  }));
...
export async function onRequestPost({ request, env }) {
  ...
  const roleLevel = Number(body.roleLevel || 1);

  if (!username || !password || !nickname) {
    return json({ error: "Preencha usuário, senha e nickname." }, 400);
  }
  ...
  if (!canCreateRole(actor, roleLevel)) {
    return json({ error: "Você não tem permissão para criar esse cargo." }, 403);
  }
  ...
  await env.DB.prepare(`
    INSERT INTO users (id, username, password, nickname, server, role_level, blocked, is_super, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)
  `).bind(id, username, password, nickname, server, roleLevel, now, now).run();

  return json({
    ok: true,
    user: { id, username, nickname, server, role_level: roleLevel, role_name: roleName(roleLevel), blocked: 0, is_super: 0, punishment_count: 0 }
  }, 201);
}
